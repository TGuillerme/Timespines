...
However, this difference only tells us armoured fishes are smaller than the others.
It doesn't tell us if they evolved armourness in relation to their body size or the body size of predators around.
...
For that we can use a kind of permutation test.

```{r}
## Running the test for the topology
## Adding the estimate ancestral values to the pooled distribution
distribution <- c(unlist(results_topol$normal.val),
                  unlist(results_topol$change.val))
## The IDs to test are the last one in the distribution
subset <- seq(from = length(results_topol$normal.val) + 1,
              to = length(distribution))

## Applying the random test for the topology
test_topol <- rand.test(distribution, subset, test = t.test,
                           replicates = 1000, test.parameter = TRUE)
...
## Adding the estimate ancestral values to the pooled distribution
distribution <- c(unlist(results_brlen$normal.val),
                  unlist(results_brlen$change.val))
## The IDs to test are the last one in the distribution
subset <- seq(from = length(results_brlen$normal.val) + 1,
              to = length(distribution))

## Applying the random test for the branch length
test_brlen <- rand.test(distribution, subset, test = t.test,
                           replicates = 1000, test.parameter = TRUE)
```
...
par(mfrow = c(2,1))
## Plotting the pooled data for every group
plot(test_topol, main = "Topology", xlab = "Mean log body size")
## Add the text
...
## Plotting the pooled data for every group
plot(test_brlen, main = "Branch length", xlab = "Mean log body size")
## Add the text
