...
```{r}
## Get the density estimates for each group
density_group <- lapply(gro_bs_norm_scal, density, na.rm = TRUE)
density_subgroup <- lapply(sub_bs_norm_scal, density, na.rm = TRUE)
density_genera <- lapply(gen_bs_norm_scal, hist) # As histograms (not enough data)
...
plot.change(gro_bs_norm_scal[[1]], gro_bs_norm_scal[[1]])
