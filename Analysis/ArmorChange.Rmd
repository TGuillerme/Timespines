...
op <- par(mfrow = c(2,3))
## Colours
# colours_groups <- as.list(rainbow(length(group_names)))
# colours_subgroups <- as.list(rainbow(length(subgroup_names)))
# colours_genera <- as.list(rainbow(length(genera_names)))

## Plotting the pooled data for every group
plot.change(gro_bs_norm_scal, gro_bs_chan_rand_scal, main = "Group - Random")
## ... subgroup ...
plot.change(sub_bs_norm_scal, sub_bs_chan_rand_scal, main = "Subgroup - Random")
## ... and genera.
plot.change(gen_bs_norm_scal, gen_bs_chan_rand_scal, main = "Genera - Random")
...
character_rand <- body_size_data$Random[match(rownames(body_size_data), rownames(body_size_data))]
character_bias <- body_size_data$Biased


test <- apply.reconstruction(placoderms_tree, character_rand, passes = 1, method = "Fitch", match.tip.char = TRUE)
